...
def design_scene():
    ...
    cfg.func("/World/deafultGroundPlane", cfg)
    # Lights
    cfg = sim_utils.DomeLightCfg(intensity=2000.0, color=(0.8, 0.8, 0.8))
    cfg.func("/World/Light", cfg)

    # Create seperate groups called "Origin1", "Origin2", "Origin3"
...
def run_simulator(sim: sim_utils.SimulationContext, entities: dict[str, DeformableObject], origins: torch.Tensor):
    ...
    # note: we only do this here for readability. In general, it is better to access the entities directly from the dictionary. This dictionary is replaced by the InteractiveScene class in the next tuturial.
    ...
    nodal_kinematics_target = cube_object.data.nodal_kinematic_target.clone()
    ...
    while simulation_app.is_running():
        ...
        if count % 250 == 0:
            ...
            # write the nodal state to the kinematic target and free all verticies
            nodal_kinematics_target[..., :3] = nodal_state[..., :3]
            nodal_kinematics_target[..., 3] = 1.0
            cube_object.write_nodal_kinematic_target_to_sim(nodal_kinematics_target)
        ...
        nodal_kinematics_target[[0, 3], 0, 2] += 0.001
        # set vertex at index 0 to be kinematically constrained
        # 0: constrained, 1: free
        nodal_kinematics_target[[0, 3], 0, 3] = 0.0
        # write kinematic target to simulation
        cube_object.write_nodal_kinematic_target_to_sim(nodal_kinematics_target)

        # [this method does not apply any external forces to the object. However, we keep this method for completeness and future extensions.
...
def main():
    """Main function"""
    # Load kit helper
    sim_cfg = sim_utils.SimulationCfg(device=args_cli.device)#, use_fabric=False)

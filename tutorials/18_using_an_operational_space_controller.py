...
class SceneCfg(InteractiveSceneCfg):
    ...
    dome_light = AssetBaseCfg(
        prim_path="/World/Light", spawn=sim_utils.DomeLightCfg
    ...
    tilted_wall = AssetBaseCfg(
        ...
        spawn=sim_utils.CuboidCfg(
            ...
            (diffuse_color=(1.0, 0.0, 0.0), opacity=0.1),
            rigid_props=sim_utils.RigidBodyPropertiesCfg
            (kinematic_enabled=True),
            activate_contact_sensors=True,
        ),
        init_state=AssetBaseCfg.InitialStateCfg(
            pos=(0.6 + 0.085, 0.0, 0.3), rot=(0.9238795325, 0.0, -0.38268343244, 0.0)
        ),
    )
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    ee_target_set = torch.cat([ee_goal_pose_set_tilted_b, 
                               ee_goal_wrench_set_tilted_task, kp_set_task], dim=-1)
...
def update_states(sim: sim_utils.SimulationContext, 
    ...
    """
    # obtian dynamics related quantities from simulation
...
# Convert the target commands to the task
def convert_to_task_frame(osc: OperationalSpaceController, command: torch.tensor, ee_target_pose_b: torch.tensor):
    ...
    for target_type in osc.cfg.target_types:
        if target_type == "pose_abs":
            ...
            # These are already defined in target frame for ee_goal_wrench_set_tilted_task (since it is easier),
            # so not transforming
            cmd_idx += 6
        else:
